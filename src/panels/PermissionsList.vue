            ...
            <el-table-column label="Global status" prop="enabled" align="right">
              <template slot-scope="scope">
                <div class="permission-delete-global">
                  <el-button type="danger" icon="el-icon-delete" circle size="mini" @click="deleteGlobalCommandConfirmation(scope.row._id)"></el-button>
                </div>
...
import { CommandPermissions, CommandPermissionsAllowed } from '../types/permissions'
...
export default class PermissionsPanel extends Vue {
  ...
  @Prop({ default: 'Fun' })
  public permissionsFilterCategory!: 'Admin' | 'ChastiKey' | 'Fun' | 'Info' | 'Integration' | 'Reddit' | 'Session' | 'User'
  ...
  private async getChannelPermissions() {
    try {
      ...
      if (resp.status === 200) {
        ...
        ;(<Array<CommandPermissions>>resp.data).map((pp) => pp.allowed.sort((a, b) => (a.name > b.name ? 1 : -1)))
  ...
  private async updateGlobalPermission(_id: string, serverID: string, $e: any) {
    console.log(_id, $e)
    const resp = await Axios(`${process.env.BOT_HOST}/permission/global/update`, {
      method: 'POST',
      data: {
        _id: _id,
        state: $e,
  ...
  private deleteGlobalCommandConfirmation(_id: string) {
    this.$bvModal
      .msgBoxConfirm("This will permanently delete the command's permissions at the global scope.  They will not be rebuilt until the bot is next rebooted.  Continue?")
      .then(async () => {
        // Send delete to API
        await this.deleteGlobalCommand(_id)

        this.$bvToast.toast('Delete completed')
      })
      .catch(() => {
        this.$bvToast.toast('Delete canceled')
      })
  }
}
</script>
